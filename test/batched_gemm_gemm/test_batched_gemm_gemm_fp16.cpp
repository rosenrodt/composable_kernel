...
TEST(TestBatchedGemmGemmInterface, GemmSpecializationSizeMismatch)
{
    ...
    // Kernel can't support odd K size because SrcVectorDim == KDim and must satisfy SizeK % A/BSrcScalarPerVector == 0
    ...
    // Kernel can't support odd O size because SrcVectorDim == ODim and must satisfy SizeO % B1SrcScalarPerVector == 0
