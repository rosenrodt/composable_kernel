...
struct GridwiseGemmGemm_xdl_cshuffle_v1
{
    ...
    struct SharedMemTrait {
        ...
        static constexpr auto a_block_desc_ak0_m_ak1 = GetABlockDescriptor_AK0PerBlock_MPerBlock_AK1();
        static constexpr auto b_block_desc_bk0_n_bk1 = GetBBlockDescriptor_BK0PerBlock_NPerBlock_BK1();
        static constexpr auto b1_block_desc_bk0_n_bk1 = GetB1BlockDescriptor_BK0PerBlock_NPerBlock_BK1();
                               ...
                               const Block2CTileMap& block_2_ctile_map)
    {
        ...
        // Initialize C
        StaticBuffer<AddressSpaceEnum::Vgpr, FloatGemmAcc, acc1_thread_buf.Size(), true> c_thread_buf;
        c_thread_buf.Clear();
        ...
        do
        {
            ...
            if (hipBlockIdx_x == 0 && hipThreadIdx_x % 32 < 4) {
                printf("bid %zd tid %zd, max = %f, sum = %f\n", hipBlockIdx_x, hipThreadIdx_x, (float)max, (float)sum);
            }
#endif
                ...
                if constexpr(num_gemm1_k_block_inner_loop > 1)
                {
                    ...
                    static_for<0, num_gemm1_k_block_inner_loop - 1, 1>{}([&](auto i) {
                        ...
                        if (hipThreadIdx_x % 32 < 8) {
                            static_for<0, acc1_thread_buf.Size(), 1>{}([&](auto I) {
                                printf("bid %zd tid %zd, iter %d, c[%d] = %f\n", hipBlockIdx_x, hipThreadIdx_x, i.value, I.value, acc1_thread_buf[I]);
                            });
                        }
#endif
                        block_sync_lds();
